...
const EditOrigin: FC<Props> = ({ formInfo, saveChanges, isLoading }) => {
  const [isEditionAvailable, setIsEditionAvailable] = useState(false);

  const form = useForm({
    initialValues: {
      province: formInfo?.origin?.province || '',
      locality: formInfo?.origin?.locality || '',
      number: formInfo?.origin?.number || '',
      street: formInfo?.origin?.street || '',
      floor: formInfo?.origin?.floor || '-',
      apartment: formInfo?.origin?.apartment || '-',
      postalCode: formInfo?.origin?.postalCode || '',
    },
    validate: {
    ...
    <form className={styles.fulfill_body_module}>
      ...
      <div>
        {isLoading ? (
          ...
          <OriginInputs form={form} isEditionAvailable={isEditionAvailable} handleChange={handleChange} />
        )}
      </div>
    </form>
  );
